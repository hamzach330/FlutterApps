...
concurrency::task<IJsonValue ^> pairRequest(JsonObject ^ command)
{
	...
	String^ code = command->GetNamedString("code", "");
	String^ _pairingKind = command->GetNamedString("pairingKind", "");
	Enumeration::DevicePairingKinds kind = Enumeration::DevicePairingKinds::ConfirmOnly;

	if (_pairingKind->Equals("ProvidePin")) {
		kind = Enumeration::DevicePairingKinds::ProvidePin;
	}
	...
	auto CustomOnPairingRequested = ref new Windows::Foundation::TypedEventHandler<Enumeration::DeviceInformationCustomPairing^, Enumeration::DevicePairingRequestedEventArgs^>(
		[code, _pairingKind](Windows::Devices::Enumeration::DeviceInformationCustomPairing^ sender,
		Windows::Devices::Enumeration::DevicePairingRequestedEventArgs^ eventArgs
	) {
		if (_pairingKind->Equals("ProvidePin")) {
			eventArgs->Accept(code);
		}
		else {
			eventArgs->Accept();
		}
	...
	// auto result = co_await device->DeviceInformation->Pairing->Custom->PairAsync();
	auto result = co_await device->DeviceInformation->Pairing->Custom->PairAsync(kind);
